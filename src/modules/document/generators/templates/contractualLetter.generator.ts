...
const normalizeParagraphs = (payload) => {
  ...
  return CONTRACTUAL_LETTER_DEFAULT_PARAGRAPHS.map(
...
export const generateContractualLetterPdfBuffer = async (
  ...
  const issueLabel = `[Date of Issuance: ${issueDate}]`;
  ...
  await drawSimpleLine(
    'Below are the key terms of your engagement:',
    helvetica,
    TYPOGRAPHY.bodySize
  );
  y -= 2;
  ...
  const keyTerms = [
    { label: 'Position Title', value: payload.jobTitle, bold: true },
    ...
    { label: 'End Date', value: payload.endDate, bold: true },
    {
    ...
    { label: 'Work Location', value: payload.workLocation || 'Remote', bold: true },
  ...
  await drawSimpleLine(payload.signatoryName, helveticaBold, TYPOGRAPHY.signatureNameSize);
  await drawSimpleLine(payload.position, helveticaBold, TYPOGRAPHY.signatureMetaSize);
