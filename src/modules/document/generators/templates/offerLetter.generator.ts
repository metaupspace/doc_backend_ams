...
export const generateOfferLetterPdfBuffer = async (
  payload,
  ...
  await drawSimpleLine({
    text: payload.greeting || `Dear ${payload.employeeName || 'Employee Name'},`,
    font: boldFont,
    size: TYPOGRAPHY.bodyHighlighted.size,
    leftX,
    lineGap: TYPOGRAPHY.body.lineGap,
  ...
  const role = payload.role || payload.positionTitle || payload.designation || payload.jobTitle;
  const introParagraph =
    typeof payload.introParagraph === 'string' && payload.introParagraph.trim()
      ? payload.introParagraph.trim()
      : OFFER_LETTER_DEFAULT_INTRO_PARAGRAPH.replace('[Department]', payload.department || 'Department')
          .replace('[Role / Position]', role || 'Role / Position');
  ...
  const keyDetailsTitle =
    typeof payload.keyDetailsTitle === 'string' && payload.keyDetailsTitle.trim()
      ...
      : 'We are pleased to extend the following offer:';
  await drawSimpleLine({
  ...
  const keyTerms = [
    { label: 'Position Title', value: payload.positionTitle || payload.designation || payload.jobTitle || payload.role },
    { label: 'Department', value: payload.department },
    { label: 'Reporting Manager', value: payload.reportingManager },
    { label: 'Probation Duration', value: payload.probationDuration || payload.probationPeriod },
    { label: 'Work Schedule', value: payload.workSchedule || 'Flexible working hours' },
    { label: 'Location', value: payload.location || payload.workLocation || 'Remote' },
    { label: 'Stipend/Compensation', value: payload.compensation || payload.stipend || payload.monthlySalary },
  ...
  const paragraphs = normalizeParagraphsWithDefaults(payload, OFFER_LETTER_DEFAULT_PARAGRAPHS);
