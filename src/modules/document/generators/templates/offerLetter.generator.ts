...
const normalizeParagraphs = (payload) => {
  ...
  return [...OFFER_LETTER_DEFAULT_PARAGRAPHS];
};
...
export const generateOfferLetterPdfBuffer = async (
  ...
  await drawSimpleLine(keyDetailsTitle, helvetica, TYPOGRAPHY.bodySize);
  y -= 2;
  ...
  const keyTerms = [
    ...
    { label: 'Probation Duration', value: payload.probationDuration, bold: true },
    { label: 'Work Schedule', value: payload.workSchedule, bold: true },
    { label: 'Location', value: payload.location || payload.workLocation, bold: true },
    {
      label: 'Stipend/Compensation',
      value: payload.compensation || payload.salaryOrStipend,
      bold: true,
    },
