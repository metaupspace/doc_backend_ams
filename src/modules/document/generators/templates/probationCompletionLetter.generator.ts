...
const normalizeParagraphs = (payload) => {
  ...
  return [...PROBATION_COMPLETION_LETTER_DEFAULT_PARAGRAPHS];
};
...
export const generateProbationCompletionLetterPdfBuffer = async (
  ...
  await drawSimpleLine(
    payload.keyDetailsTitle || 'Below are the key details of your employment confirmation:',
    helvetica,
    TYPOGRAPHY.bodySize
  );
  y -= 2;
  ...
  const keyTerms = [
    { label: 'Probation Start Date', value: payload.probationStartDate, bold: true },
    { label: 'Probation End Date', value: payload.probationEndDate, bold: true },
    { label: 'Confirmation Effective Date', value: payload.confirmationEffectiveDate, bold: true },
    { label: 'Job Title', value: payload.jobTitle || payload.designation, bold: true },
    ...
    { label: 'Stipend', value: payload.stipend, bold: true },
