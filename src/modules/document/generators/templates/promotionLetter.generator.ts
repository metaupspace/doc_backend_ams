...
const resolvePromotionTemplate = (template, payload, companyName) => {
  const salaryValue = payload.revisedSalary || payload.salary || '[New Salary]';
  const salaryFrequency = payload.salaryFrequency || 'month/year';

  return String(template)
    .replace(/MetaUpSpace LLP/g, companyName)
    .replace('[New Job Title]', payload.newJobTitle || payload.newDesignation || '[New Job Title]')
    .replace('[Department]', payload.department || '[Department]')
    .replace('[Effective Date]', payload.effectiveFrom || payload.effectiveDate || '[Effective Date]')
    .replace('[New Salary]', salaryValue)
    .replace('[month/year]', salaryFrequency)
    .replace('[Salary Effective Date]', payload.salaryEffectiveDate || payload.effectiveFrom || payload.effectiveDate || '[Salary Effective Date]');
};
...
export const generatePromotionLetterPdfBuffer = async (
  ...
  const introParagraph =
    typeof payload.introParagraph === 'string' && payload.introParagraph.trim()
      ...
      : resolvePromotionTemplate(PROMOTION_LETTER_DEFAULT_INTRO_PARAGRAPH, payload, companyName);

  await drawParagraph(introParagraph);

  const paragraphs = normalizeParagraphsWithDefaults(payload, PROMOTION_LETTER_DEFAULT_PARAGRAPHS).map(
    (paragraph) => resolvePromotionTemplate(paragraph, payload, companyName)
  );
