...
const normalizeParagraphs = (payload) => {
  ...
  return [...RESIGNATION_ACCEPTANCE_LETTER_DEFAULT_PARAGRAPHS];
};
...
export const generateResignationAcceptanceLetterPdfBuffer = async (
  ...
  await drawSimpleLine(
    `Dear ${payload.employeeName || `Candidate's name`},`,
    helveticaBold,
  ...
  const introParagraph =
    typeof payload.introParagraph === 'string' && payload.introParagraph.trim()
      ...
      : `This letter formally acknowledges receipt of your resignation submitted on ${
          payload.resignationDate || '[Insert Date]'
        } at ${payload.resignationTime || '[Insert Time]'}, from your position as ${
          payload.jobTitle || '[Job Title]'
        } with ${
          payload.companyName || COMPANY_NAME
        }. As per your notice, your resignation will be effective from ${
          payload.effectiveDate || '[Insert Effective Date]'
        }, which will also serve as your final working day with the organization.`;
