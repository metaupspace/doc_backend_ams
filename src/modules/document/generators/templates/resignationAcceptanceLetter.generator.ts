...
const resolveResignationIntroTemplate = (template, payload, companyName) => {
  const resignationDate = payload.resignationDate || '[Insert Date]';
  const resignationTime = payload.resignationTime || '[Insert Time]';
  const jobTitle = payload.jobTitle || '[Job Title]';
  const effectiveDate = payload.effectiveDate || '[Insert Effective Date]';

  return String(template)
    .replace(/MetaUpSpace LLP/g, companyName)
    .replace('[Insert Date]', resignationDate)
    .replace('[Insert Time]', resignationTime)
    .replace('[Job Title]', jobTitle)
    .replace('[Insert Effective Date]', effectiveDate);
};

export const generateResignationAcceptanceLetterPdfBuffer = async (
  payload,
  ...
  const title = payload.title || 'RESIGNATION ACCEPTANCE LETTER';
  ...
  await drawSimpleLine({
    text: payload.greeting || `Dear ${payload.employeeName || "Candidate's name"},`,
    font: boldFont,
    size: TYPOGRAPHY.bodyHighlighted.size,
    leftX,
    lineGap: TYPOGRAPHY.body.lineGap,
  ...
  const introParagraph =
    typeof payload.introParagraph === 'string' && payload.introParagraph.trim()
      ...
      : resolveResignationIntroTemplate(
          RESIGNATION_ACCEPTANCE_LETTER_DEFAULT_INTRO_PARAGRAPH,
          payload,
          companyName
        );
