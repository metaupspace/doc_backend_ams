...
const resolveTerminationTemplate = (template, payload, companyName) => {
  const lastWorkingDate = payload.lastWorkingDate || '[Last Working Date]';

  return String(template)
    .replace(/MetaUpSpace LLP/g, companyName)
    .replace('[Last Working Date]', lastWorkingDate);
};

const getSettlementItems = (payload) => {
  if (Array.isArray(payload.settlementItems) && payload.settlementItems.length > 0) {
    return payload.settlementItems.filter(Boolean);
  }

  const lastOfficialWorkingDate =
    payload.lastOfficialWorkingDate || payload.lastWorkingDate || '[Last official working date]';
  const finalSettlementDays = payload.finalSettlementDays || 45;

  return [
    `Salary up to your last working day i.e. <strong>${lastOfficialWorkingDate}</strong>`,
    'Any unpaid reimbursements or earned leave encashment (if applicable), in accordance with company policy.',
    `Full and final settlement within <strong>${finalSettlementDays} days</strong> of your last working date.`,
  ];
};
...
export const generateTerminationLetterPdfBuffer = async (
  ...
  const title = payload.title || 'TERMINATION LETTER';
  ...
  const issueText =
    payload.issueDate || issuedAt
      ? formatIssueDate(payload.issueDate || issuedAt)
      : '[Date of Issuance]';
  ...
  const introParagraph =
    typeof payload.introParagraph === 'string' && payload.introParagraph.trim()
      ...
      : resolveTerminationTemplate(TERMINATION_LETTER_DEFAULT_INTRO_PARAGRAPH, payload, companyName);

  await drawParagraph(introParagraph);

  const paragraphs = normalizeParagraphsWithDefaults(payload, TERMINATION_LETTER_DEFAULT_PARAGRAPHS);

  if (paragraphs.length > 0) {
    await drawParagraph(paragraphs[0]);
  }

  await drawSimpleLine({
    text: payload.settlementIntro || 'You will be entitled to receive:',
    font: regularFont,
    size: TYPOGRAPHY.body.size,
    leftX,
    lineGap: TYPOGRAPHY.body.lineGap,
  ...
  const settlementItems = getSettlementItems(payload);
  for (const item of settlementItems) {
    await drawParagraph(`• ${item}`, TYPOGRAPHY.body.size, 4);
  }
