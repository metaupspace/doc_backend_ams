...
const resolveWarningIntroTemplate = (template, payload) => {
  const warningReason = payload.warningReason || payload.reason;
  if (!warningReason) {
    return String(template);
  }

  return String(template).replace(
    '(briefly describe the specific elements of behaviour or performance that are unsatisfactory or in breach of policy)',
    warningReason
  );
};
...
export const generateWarningLetterPdfBuffer = async (
  payload,
  ...
  const title = payload.title || 'FIRST WARNING LETTER';
  ...
  const introParagraph =
    typeof payload.introParagraph === 'string' && payload.introParagraph.trim()
      ...
      : resolveWarningIntroTemplate(WARNING_LETTER_DEFAULT_INTRO_PARAGRAPH, payload);

  await drawParagraph(introParagraph);

  const paragraphs = normalizeParagraphsWithDefaults(payload, WARNING_LETTER_DEFAULT_PARAGRAPHS).map(
    (paragraph) => {
      const companyName = payload.companyName || COMPANY_NAME;
      return String(paragraph).replace(/MetaUpSpace LLP/g, companyName);
    }
  );
  ...
  await drawSimpleLine({
    text: payload.signatoryCompany || payload.companyName || COMPANY_NAME,
    font: boldFont,
    size: TYPOGRAPHY.bodyHighlighted.size,
    leftX,
    lineGap: TYPOGRAPHY.body.lineGap,
